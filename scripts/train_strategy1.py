"""Strategy 1 training: causal chain naturalization.

The agent learns to execute causal chains (detect→investigate→fix→verify)
instead of isolated actions. Each chain has a logical narrative that makes
the defender see "normal admin behavior" not "attacker actions".
"""
...
from src.causal_chains import CAUSAL_CHAINS, CAUSAL_COMMANDS, CausalTimingPolicy
...
def run_sim_episode(env, policy, max_steps=200):
    ...
    while not done:
        ...
        # Clamp to sim-supported actions
        if action > 4:
...
def main():
    logger.info("=" * 70)
    logger.info("STRATEGY 1: Causal Chain Naturalization")
    logger.info("=" * 70)
    ...
    logger.info(f"\n{'='*70}")
    logger.info("Phase 1: ES on simulation (causal chains)")
    logger.info(f"{'='*70}")
    ...
    for gen in range(n_gen):
        ...
        for i in range(pop):
            ...
            policy = CausalTimingPolicy(candidate)
        ...
        eval_policy = CausalTimingPolicy(base_params)
    ...
    # ── Summary ──
    logger.info(f"\n{'='*70}")
    logger.info("STRATEGY 1 RESULTS")
    logger.info(f"{'='*70}")
    ...
    logger.info(f"\n  COMPARISON:")
    logger.info(f"    Isolated actions (prev):  reward=56.92  tasks=17.0  det=0/3")
    logger.info(f"    Causal chains (new):      reward={np.mean(rewards):.2f}  "
    ...
    }, "checkpoints/strategy1_causal.pt")
    logger.info("\n✅ Strategy 1 complete. Saved → checkpoints/strategy1_causal.pt")
